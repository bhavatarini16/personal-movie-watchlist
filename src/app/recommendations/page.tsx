...
export default function RecommendationsPage() {
  ...
  const getRecommendations = async () => {
    setLoading(true);
    try {
      const watched = MOCK_MOVIES.filter(m => m.status === 'watched').map(m => ({
        title: m.title,
        genres: m.genres,
        rating: m.rating || 5
      }));
      ...
      {!recommendations ? (
        <div className="h-[400px] glass border-white/5 rounded-3xl flex flex-col items-center justify-center text-center p-8">
          ...
          <p className="text-white/50 max-w-md mx-auto mb-8">
            Click the button above and our AI will analyze your watch history and ratings to suggest movies you'll actually love.
          </p>
        </div>
      ) : (
          ...
          {recommendations.map((rec, idx) => (
            <Card key={idx} className="glass border-white/5 group hover:border-primary/40 transition-all overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: `${idx * 150}ms` }}>
              <div className="aspect-video relative overflow-hidden">
                <Image 
                  src={`https://picsum.photos/seed/rec${idx}/600/400`} 
                  alt={rec.title}
                  fill
